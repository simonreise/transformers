...
class DPTImageProcessor(BaseImageProcessor):
    ...
    def preprocess(
        ...
        if segmentation_maps is not None:
            ...
            if do_reduce_labels:
                segmentation_maps = [
                    self.reduce_label(segmentation_map)
            ...
            if do_normalize:
                segmentation_maps = [
                    self.normalize(image=segmentation_map, mean=image_mean, std=image_std, input_data_format=input_data_format)
                    for segmentation_map in segmentation_maps
                ]
    
            if do_pad:
                segmentation_maps = [
                    self.pad_image(image=segmentation_map, size_divisor=size_divisor, input_data_format=input_data_format)
                    for segmentation_map in segmentation_maps
                ]
